...
if(isset($_REQUEST['buscarVenda']) && $_REQUEST['buscarVenda']== ''){
  Redirect::to('Lista_factura.php');
}
if(isset($_REQUEST['data_de']) || isset($_REQUEST['data_a'])){
  if($_REQUEST['data_de']=='' || $_REQUEST['data_a']==''){
     Redirect::to('Lista_factura.php');
  }
}

if(!empty($_REQUEST['buscarVenda'])){
$buscarVenda=strtolower($_REQUEST['buscarVenda']);
$where="nr_factura=$buscarVenda";
//$buscar="buscarVenda=$buscarVenda";
}

if(!empty($_REQUEST['data_de']) && !empty($_REQUEST['data_a'])){
  ...
  else if($data_de == $data_a){
   $where="dataVenda LIKE '$data_de%'";
    //$buscar="data_de=$data_de&data_a=$data_a";
    }
    else{
      $f_de=$data_de.' 00:00:00';
      $f_a=$data_a.' 23:59:59';
     $where="dataVenda BETWEEN '$f_de' AND '$f_a'";
      //$buscar="data_de=$data_de&data_a=$data_a";
    }
...
<ul class="navbar-nav">
  ...
  <div class="content-wrapper">
      ...
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1><i class="far fa-newspaper"></i>Lista facturas</h1>
          </div>
          <div class="col-sm-6">
          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                 ...
                 <form action="buscar_venda.php" method="get" >
             <div class="form-inline dados_inline">
                  <a href="factura.php" ><i class="fas fa-user-plus"></i>Nova factura</a>
                <input type="number" class="form-control" name="buscarVenda" id="buscarVenda" placeholder="Número factura"  autocomplete required>
...
<table class="table table-borderless" cellpadding="10" cellspacing="10">
  <form action="buscar_venda.php" method="get" class="form_search_date">
...
$query= mysqli_query($conexao,"SELECT f.nr_factura,fun.name,
                              ...
                              f.total_factura FROM tb_factura f INNER JOIN 
                              users fun ON f.usuario=fun.id
                               INNER JOIN tb_cliente cl ON f.cod_cliente=cl.idCliente WHERE $where");
      ...
      while($data=mysqli_fetch_array($query)){
        ...
        if($data['estado']==1) {
   $estado='<span class="pagada">Paga</span>';
 }
 else{
...
if($data['estado']==1) {
 ...
 <div class="div_factura">
   <button class="btn_anular anular_factura"  fac="<?php echo $data['nr_factura']; ?>"><i class="fas fa-ban"></i></button>
 </div>
      <?php
      }
      else{
