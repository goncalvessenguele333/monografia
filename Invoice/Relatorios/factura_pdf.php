...
foreach ($dados->listaDadosEmpresa(12342018) as $lista) {
  ...
  $pdf->SetFont('Times','B',11);  
    ...
    $pdf->Cell(120,10,escape($lista->endereco),0,0,'L');
      ...
      $pdf->Ln(15);
      $pdf->SetDrawColor(63,171,158);
      $pdf->SetLineWidth(0.5);
      $pdf->Line(10,89,198,89);
    $pdf->SetTextColor(0,0,0);   
    $pdf->SetFont('Times','B',11);   

    $pdf->Cell(60,7,"V/Nuit",0,0,'L');
    $pdf->Cell(30,7,utf8_decode("Requisição"),0,0,'C');
    $pdf->Cell(30,7,"Nr do Cliente",0,0,'C');
 ...
 $pdf->SetTextColor(0,0,0);  
    ...
    $codigoF= ($dados1->data()->id); 
     ...
     foreach ($dados->PesquisaCliente($codCl) as $lista) {
  $pdf->Cell(60,7,escape($lista->nuitCliente),0,0,'L');
}   
     //$pdf->Line(45,45,200,45);
       $pdf->Cell(30,7,"",0,0,'C'); 
      foreach ($dados->PesquisaCliente($codCl) as $lista) {
  $pdf->Cell(30,7,escape($lista->idCliente),0,0,'C');
}   
     $pdf->Cell(35,7,"MZN",0,0,'C');  
      $pdf->Cell(35,7,"1.00",0,0,'C');
      $pdf->Ln(2);

       $pdf->Cell(60,7,"____________________",0,0,'L');
       $pdf->Cell(30,7,"______________",0,0,'L');     
      $pdf->Cell(30,7,"_______________",0,0,'C');  
...
$pdf->SetTextColor(0,0,0);
 $pdf->SetFont('Times','B',11);
    $pdf->Cell(50,7,"Desc. Cliente",0,0,'L');
    $pdf->Cell(40,7,utf8_decode("Condição de pagamento"),0,0,'C');
    $pdf->Cell(40,7,"Vencimento",0,0,'C');
    $pdf->Cell(30,7,"",0,0,'C');
    $pdf->Cell(30,7,"",0,0,'C');  
    $pdf->Ln();
 $pdf->SetTextColor(0,0,0);  
  ...
  $pdf->Cell(50,7,"0,00",0,0,'C');
    $pdf->Cell(40,7,"Factura 07 dias",0,0,'C');     
      $pdf->Cell(40,7,"",0,0,'C');  
     $pdf->Cell(30,7,"",0,0,'L');  
      $pdf->Cell(30,7,"",0,0,'C');
      $pdf->Ln(3);
       $pdf->Cell(50,7,"____________________",0,0,'L');
       $pdf->Cell(40,7,"____________________",0,0,'C');     
      $pdf->Cell(40,7,"____________________",0,0,'C');  
     $pdf->Cell(30,7,"",0,0,'L');  
      $pdf->Cell(30,7,"",0,0,'C');
       $pdf->Ln(5);
       ...
       // $pdf->SetFillColor(192,192,192);
        ...
        $pdf->Cell(110,9,utf8_decode('Descrição'),0,0,'C',1);
         $pdf->Cell(12,9,'Qtd.',0,0,'C',1);
        $pdf->Cell(23,9,utf8_decode('P. Unitário'),0,0,'C',1);
...
foreach ($prod->detalheFactura($coFact) as $lista) { 
          ...
          $xPos=$pdf->GetX();
           $yPos=$pdf->GetY();
            ...
            $pdf->MultiCell(110,5,utf8_decode($lista->description),0);
