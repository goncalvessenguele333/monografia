...
foreach ($dados->listaDadosEmpresa(12342018) as $lista) {
 ...
 foreach ($dados->PesquisaCliente($codCl) as $lista) {
   $this->SetFont('Times','B',14);
    ...
    $this->Cell(10,10,utf8_decode('COTAÇÃO'),0,0,'C');
    $this->Ln(10);
    $this->SetFont('Times','B',11);
    $this->Cell(100,10,'',0,0,'C');
  $this->Cell(20,10,'Nro:',0,0,'L');
    foreach ($prod->detalheTotal_cotacao($coFact) as $lista1) {     
        $this->Cell(60,10,'MS/SQ - '.$coFact.'/'.escape($lista1->ano),0,0,'L');  
     } 
  
    $this->Ln(7);  
    $this->SetFont('Times','B',11);
     $this->Cell(100,10,'',0,0,'C');   
  $this->Cell(20,10,utf8_decode('Endereço:'),0,0,'L');
...
$pdf->AddPage(); 
  ...
  $pdf->Cell(50,7,utf8_decode($lista->name),1,0,'C');
     ...
     foreach ($prod->detalheTotal_cotacao($coFact) as $lista) {     
       $pdf->Cell(30,7,escape($lista->dataVenda),1,0,'C');
     } 
     ...
     $pdf->Cell(25,7,"Pronto",1,0,'C');  
      $pdf->Cell(20,7,"07 dias",1,0,'C');
      $pdf->Ln(3);
      ...
      $pdf->SetFillColor(255,255,255);  
        ...
        $pdf->SetFont('Times','B',10); 
          $pdf->SetLineWidth(0.3);
         $pdf->Cell(15,9,'Ordem',1,0,'C',1);
        $pdf->Cell(113,9,utf8_decode('Descrição/Refª'),1,0,'C',1);
         $pdf->Cell(12,9,'Qtd.',1,0,'C',1);
        $pdf->Cell(23,9,utf8_decode('Preço Unit.'),1,0,'C',1);
         $pdf->Cell(25,9,utf8_decode('Total Líquido'),1,0,'C',1);
...
foreach ($prod->detalheCotacao($coFact) as $lista) { 
 ...
 $pdf->Ln(5); 
        ...
        $pdf->SetFont('Times','I',10);
         ...
         foreach ($dados->PesquisaCliente($codCl) as $listaI) {
          $pdf->Cell(20,3,escape($listaI->iva).'%',0,0,'L',1);
           ...
           $pdf->Cell(25,3,escape($lista->subtotal_factura),0,0,'L',1);
      ...
      $pdf->Ln(5);
       $pdf->SetFont('Times','B',10);
          $pdf->Cell(170,3,'TOTAL(MT): ',0,0,'R',1);
 ...
 $pdf->Cell(60,8,'____________________________',0,0,'C'); 
  ...
  $pdf->Cell(130,8,' IBAM: MZ59000301120858168100396 | SWIFT: SBICMZMX | C/Móvel: 833211705',0,0,'L');
